# Eze-Cart — User Roles & Permissions
...
|------|-----|-------|-----------|
| **Main Admin** | Admin Panel (Next.js) | Entire system, all Service Locations | Seeded / system owner |
| **Delivery Staff** | Staff App (Flutter) | One assigned Service Location (optionally category-restricted) | Main Admin |
| **Customer** | Customer App (Flutter) | Their own account + one chosen Service Location's catalog | Self-registration |
...
- Manage Categories per Service Location (add/remove, image upload).
- Manage Products per Category (add/remove; name, gross weight, net weight, price; image upload).
- Manage Delivery Time options (add/remove).
- Manage Staff accounts and their granular accessibility/permissions.
...
- View Total Sales reporting by Service Location.
- Look up customer purchase history by Name / WhatsApp number.

**Data ownership:** All entities. Full read/write.

**Accessible app:** Admin Panel only.

## 3. Delivery Staff

**Responsibilities**
- Log in (tied to a Service Location).
- View orders for their assigned location (optionally filtered to specific categories per Admin config).
- Progress each order through its lifecycle:
  - **Order Confirm** — acknowledge/accept the order.
  - **Delivery Confirm** — mark the order delivered.
  - **Payment Confirm** — record payment collected and its method (Gpay or Cash).
- Manage their own profile.

**Configurable (granular) permissions — set per staff member by Main Admin:**
| Permission | Description | Default |
|------------|-------------|---------|
| `view_orders` | See orders for assigned location | On |
...
**Data ownership:** Orders within assigned location (and allowed categories). Read on catalog for their location; write on products only if `add_products` granted.

**Accessible app:** Staff App (as staff). A staff member may **also** use the Customer App as a normal customer — see §8.
...
- Mobile (WhatsApp) number is the identity and **must be OTP-verified** (OTP via AWS SNS). Numbers are **unique**.
...
| View Orders (own location) | ✅ | ✅ | ❌ (own orders only) |
...
- A Staff member only sees orders for **their** assigned Service Location (and allowed categories).
...
- **Customer App** authenticates against the **Customer identity** (phone + OTP-verified session, cached/auto-login). No password.
- **Staff App** authenticates against the **Staff identity** (phone **+ password**, credentials created by Admin).
- Customer and Staff are **separate records/tables** keyed on phone; the same phone value may exist once in each. There is no cross-linking requirement — a person can act as a customer in the Customer App and as staff in the Staff App using the same number.
- **Resolution rule:** the app being used determines the identity resolved. The Customer App never resolves a staff role, and the Staff App never resolves a customer role. Uniqueness is enforced **within** each identity type (unique customer phone; unique staff phone), not across them.
